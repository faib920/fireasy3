...
namespace Fireasy.Data.Batcher
{
    ...
    public abstract class BatcherBase
    {
        ...
        protected IList<PropertyFieldMapping> GetNameTypeMapping(object item)
        {
            ...
            foreach (PropertyDescriptor pd in TypeDescriptor.GetProperties(item))
            {
                ...
                {
                    throw new InvalidOperationException($"类型 {item.GetType()} 的属性 {pd.Name} 的类型为空。");
                }
        ...
        protected void BatchSplitData(ICollection collection, int batchSize, Action<int, int, object> rowAction, Action<int, int, int, bool> splitAction, CancellationToken cancellationToken = default)
        {
            int batch = 0, index = 0;
    ...
    {
        public BatchOperator(Func<DbConnection, T> openFunc, Func<DbCommand, T> executeFunc, Func<T> @return = null)
        {
            OpenFunc = openFunc;
        ...
        internal Func<T> Return { get; set; }
    }
}
