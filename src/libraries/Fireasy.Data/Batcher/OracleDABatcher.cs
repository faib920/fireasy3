...
namespace Fireasy.Data.Batcher
{
    ...
    public sealed class OracleDABatcher : BatcherBase, IBatcherProvider
    {
        ...
        public async Task InsertAsync(IDatabase database, DataTable dataTable, int batchSize = 1000, Action<int>? completePercentage = null, CancellationToken cancellationToken = default)
        {
            ...
            if (bulkType != null)
            {
                await BulkCopyAsync(database, bulkType, dataTable, batchSize);
            }
            else
            {
        ...
        public async Task InsertAsync<T>(IDatabase database, IEnumerable<T> list, string tableName, int batchSize = 1000, Action<int>? completePercentage = null, CancellationToken cancellationToken = default)
        {
            ...
            if (bulkType != null)
            {
                await BulkCopyAsync(database, bulkType, list.ToDataTable(tableName), batchSize);
            }
            else
            {
        ...
        /// <param name="batchSize">每批次写入的数据量。</param>
        /// <returns></returns>
        private async Task<bool> BulkCopyAsync(IDatabase database, Type bulkType, DataTable table, int batchSize)
        {
            using var connection = database.CreateConnection();
            using var bulk = Activator.CreateInstance(bulkType, connection) as IDisposable;
            bulkType.GetProperty("DestinationTableName")?.SetValue(bulk, table.TableName, null);
            bulkType.GetProperty("BatchSize")?.SetValue(bulk, batchSize, null);

            try
            {
                await connection.TryOpenAsync();
                bulkType.GetMethod("WriteToServer")?.Invoke(bulk, new object[] { table });
