...
namespace Fireasy.Data.Syntax
{
    ...
    /// MySql字符串函数语法解析。
    /// </summary>
    public class MySqlStringSyntax : IStringSyntax
    {
        /// <summary>
        /// 取源表达式的子串。
        /// </summary>
        ...
        {
            return $"LPAD({sourceExp}, LENGTH({sourceExp}) + {count}, {padding ?? "' '"})";
        }
        ...
        {
            return $"RPAD({sourceExp}, LENGTH({sourceExp}) + {count}, {padding ?? "' '"})";
        }
        ...
        public virtual string GroupConcat(object sourceExp, object separator, object? orderby)
        {
            if (orderby != null)
            {
                return $"GROUP_CONCAT(DISTINCT {sourceExp} ORDER BY {orderby} SEPARATOR {separator})";
            }
