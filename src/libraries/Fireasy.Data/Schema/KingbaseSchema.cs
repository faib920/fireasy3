...
WHERE (TC.CONSTRAINT_CATALOG = '{connpar.Database}' AND TC.TABLE_SCHEMA = ANY (CURRENT_SCHEMAS(false)))
  AND CONSTRAINT_TYPE = 'FOREIGN KEY'{(parameters.HasValue("TABLENAME") ? @"
  AND TC.TABLE_NAME IN (@TABLENAME)" : string.Empty)}{(parameters.HasValue("COLUMNNAME") ? @"
...
WHERE (TABLE_CATALOG = '{connpar.Database}' AND TABLE_SCHEMA = ANY (CURRENT_SCHEMAS(false))){(parameters.HasValue("NAME") ? @"
  AND T.TABLE_NAME IN (@NAME)" : string.Empty)}
 ORDER BY T.TABLE_CATALOG, T.TABLE_SCHEMA, T.TABLE_NAME";
...
JOIN (
  ...
  WHERE RELKIND = 'v' AND RELNAME NOT LIKE 'pg_%' AND RELNAME NOT LIKE 'sql_%'
    ...
    ON col.TABLE_NAME = C.TABLE_NAME
WHERE (COL.TABLE_CATALOG = '{connpar.Database}' AND COL.TABLE_SCHEMA = ANY (CURRENT_SCHEMAS(false))){(parameters.HasValue("VIEWNAME") ? @"
  AND COL.TABLE_NAME IN (@VIEWNAME)" : string.Empty)}{(parameters.HasValue("COLUMNNAME") ? @"
  AND COL.COLUMN_NAME IN (@COLUMNNAME)" : string.Empty)}
ORDER BY
  COL.TABLE_CATALOG, COL.TABLE_SCHEMA, COL.TABLE_NAME, COL.ORDINAL_POSITION";
