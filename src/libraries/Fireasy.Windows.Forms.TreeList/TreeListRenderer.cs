...
namespace Fireasy.Windows.Forms
{
    ...
    public class TreeListRenderer : ControlRenderer
    {
        ...
        public virtual void DrawColumnHeader(TreeListColumnRenderEventArgs e)
        {
            ...
            if (e.Column?.Index == 0 && e.Column.TreeList.ShowCheckAllBoxOnHeader)
            {
                var h = (e.Bounds.Height - 15) / 2;
                checkRect = new Rectangle(e.Bounds.X + 2, e.Bounds.Top + h, 15, 15);
                textBound.X += checkRect.Width + 2;
                textBound.Width -= checkRect.Width - 2;

                if (!imageRect.IsEmpty)
                {
                    imageRect.X += checkRect.Width + 2;
                }
            }
            ...
            {
                ThemeManager.BaseRenderer.DrawCheckbox(e.Graphics, checkRect, e.Column.TreeList.CheckAllChecked, false, true);
            }
        ...
        protected virtual void DrawItemBackground(TreeListItemRenderEventArgs e)
        {
            ...
            if (e.DrawState == DrawState.DragTop)
            {
                e.Graphics.DrawLine(Pens.DeepSkyBlue, e.Bounds.Left, e.Bounds.Top, e.Bounds.Right - 1, e.Bounds.Top);
            }
            else if (e.DrawState == DrawState.DragBottom)
            {
                e.Graphics.DrawLine(Pens.DeepSkyBlue, e.Bounds.Left, e.Bounds.Bottom - 1, e.Bounds.Right - 1, e.Bounds.Bottom - 1);
            }
        }
        ...
        {
            if (e.DrawState == DrawState.Drag || e.DrawState == DrawState.DragTop || e.DrawState == DrawState.DragBottom)
            {
