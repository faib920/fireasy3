...
namespace Fireasy.Windows.Forms
{
    public partial class TreeList
    {
        ...
        [Description("当列头单击全选时触发此事件。")]
        public event TreeListCheckAllEventHandler CheckAllChanged;
        [Description("当行拖拽经过另一行时触发此事件。")]
        public event TreeListItemDragOverEventHandler ItemDragOver;
        [Description("当行拖拽到另一行时触发此事件。")]
        public event TreeListBeforeItemDragDownEventHandler BeforeItemDragDown;
        [Description("当行拖拽到另一行时触发此事件。")]
        public event TreeListAfterItemDragDownEventHandler AfterItemDragDown;
        ...
        protected internal void RaiseCheckAllChangedEvent(TreeListColumn column, bool @checked)
        {
            if (CheckAllChanged == null)
            {
                return;
            }

            var e = new TreeListCheckAllEventArgs { Column = column, Checked = @checked };
            OnCheckAllChanged(e);
        }
        ...
        protected virtual void OnCheckAllChanged(TreeListCheckAllEventArgs e)
        {
            CheckAllChanged?.Invoke(this, e);
        }
        ...
        protected virtual void OnItemDragOver(TreeListItemDragOverEventArgs e)
        {
            ItemDragOver?.Invoke(this, e);
        }

        protected virtual void OnBeforeItemDragDown(TreeListBeforeItemDragDownEventArgs e)
        {
            BeforeItemDragDown?.Invoke(this, e);
        }

        protected virtual void OnAfterItemDragDown(TreeListAfterItemDragDownEventArgs e)
        {
            AfterItemDragDown?.Invoke(this, e);
        }
        #endregion
    }
}
