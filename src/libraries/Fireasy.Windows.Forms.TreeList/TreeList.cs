...
namespace Fireasy.Windows.Forms
{
    ...
    public partial class TreeList : Control, IBorderStylization, IBackgroundAligning
    {
        ...
        private TreeListHitTestInfo _lastDragingHitInfo;
        private DrawState _lastDragStatus;
        private bool _lastDragdownable;
        ...
        public TreeListRenderer Renderer
        {
            get
            {
                return _renderer ?? (_renderer = new TreeListRenderer());
            }
            set
            {
                _renderer = value;
        ...
        {
            get { return _vbar.Visible; }
        }

        internal bool ShowHorScrollBar
        {
            get { return _hbar.Visible; }
        }
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            {
                if (_lastHoverHitInfo.HitTestType == TreeListHitTestType.ColumnSize)
                {
                ...
                else if (AllowDragItem && _lastHoverHitInfo.HitTestType == TreeListHitTestType.Cell)
                {
                    var current = HitTest(e.X, e.Y, TreeListHitTestEventType.MouseMove);
                    ProcessHitTestInfoDragOver(current, e.Location);
                }
            }
            else
            {
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            else if (AllowDragItem && _lastDragingHitInfo != null)
            {
                ProcessItemDragDown();
            }
            else if (e.Button == MouseButtons.Left)
            {
                ...
                if (current != null)
                {
                    ...
                    if (_lastHoverHitInfo != null && !_lastHoverHitInfo.Equals(current))
                    {
        ...
        {
            if (m.Msg == (int)NativeMethods.Msgs.W_SHOWWINDOW)
            {
        ...
        public void BeginUpdate()
        {
            ...
            NativeMethods.SendMessage(Handle, (int)NativeMethods.Msgs.W_SETREDRAW, 0, 0);
        }
        ...
        public void EndUpdate()
        {
            _isUpdating = false;
            _lastRowIndex = -1;
            NativeMethods.SendMessage(Handle, (int)NativeMethods.Msgs.W_SETREDRAW, 1, 0);
            UpdateItems();
        }
        ...
        private void SetScrollBars()
        {
            ...
            if (!showHBar)
            {
                showVBar = height > _bound.WorkBound.Height - _bound.ColumnBound.Height - (ShowFooter ? FooterHeight : 0);
            }
            if (!showVBar)
            {
                showHBar = width > _bound.WorkBound.Width;
            }

            if (showVBar)
            {
                _vbar.Location = new Point(Width - _vbar.Width - borderWidth, borderWidth);
                _vbar.Height = Height - borderWidth * 2 - (showHBar ? _hbar.Height : 0);
                _vbar.Maximum = Math.Max(0, height + (showHBar ? _hbar.Height : 0) - _bound.ItemBound.Height);
                _vbar.LargeChange = _vbar.Maximum / 5;
            ...
            if (showHBar)
            {
                ...
                _hbar.Width = Width - borderWidth * 2 - (showVBar ? _vbar.Width : 0);
                _hbar.Maximum = Math.Max(0, width + (showVBar ? _vbar.Width : 0) - _bound.ItemBound.Width);
                _hbar.LargeChange = _hbar.Maximum / 5;
                _hbar.Maximum += _vbar.LargeChange;
                _hbar.Visible = true;
            }
            else
            {
