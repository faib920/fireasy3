...
namespace Fireasy.Common.Emit
{
    ...
    public sealed class EmitHelper
    {
        ...
        /// 开始 Catch 块。
        /// </summary>
        /// <param name="exceptionType">表示异常的 System.ModuleType 对象。</param>
        ...
        /// 声明指定类型的局部变量。
        /// </summary>
        /// <param name="localType">一个 System.ModuleType 对象，表示局部变量的类型。</param>
        ...
        /// 将值类转换为对象引用（O 类型）。
        /// </summary>
        /// <param name="type">A ModuleType</param>
        /// <returns>当前 <see cref="EmitHelper"/> 的实例。</returns>
        ...
        /// <returns>当前 <see cref="EmitHelper"/> 的实例。</returns>
        public EmitHelper If(bool predicate, Action<EmitHelper> trueAction, Action<EmitHelper>? falseAction = null)
        {
            if (predicate)
            {
        ...
        /// <returns></returns>
        public EmitHelper Default(Type valueType)
        {
            if (valueType.IsPrimitive)
            {
                if (valueType == typeof(long) || valueType == typeof(ulong))
                {
                    return this.ldc_i4_0.conv_i8;
                }
                if (valueType == typeof(float))
                {
                    return this.ldc_r4(0);
                }
                if (valueType == typeof(double))
                {
                    return this.ldc_r8(0);
                }
            ...
            {
                var lb = this.DeclareLocal(valueType);
                return this.ldloca(lb).initobj(valueType).ldloc((short)lb.LocalIndex);
            }
        ...
        {
            return new InvalidOperationException($"未找到指定的方法 {methodName}。");
        }

        private static Exception ThrowNotExpectedTypeException(Type type)
        {
            return new ArgumentException($"非预期的类型 {type.FullName}。");
        }
    }
}
