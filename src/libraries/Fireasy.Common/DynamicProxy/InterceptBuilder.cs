...
namespace Fireasy.Common.DynamicProxy
{
    ...
    public static class InterceptBuilder
    {
        ...
        public static Type BuildType(Type type, InterceptBuildOption? option = null)
        {
            ...
            if (type.IsSealed)
            {
                throw new DynamicProxyException($"Aspect 无法为类型 {type.FullName} 创建代理类，因为该类型是密封的。");
            }
            ...
            builder.ImplementInterface(typeof(IDynamicProxyImplemented));
            ...
            {
                throw new DynamicProxyException("Aspect 无法为类型 {type.FullName} 创建代理类。", exp);
            }
        }
        ...
        private static DynamicMethodBuilder InjectGenericMethod(BuildInternalContext context, MethodInfo method, IEnumerable<InterceptAttribute> attributes)
        {
            ...
            var methodBuilder = context.TypeBuilder.DefineMethod(
                ...
                ilCoding: ctx =>
                {
                    ...
                    ctx.Emitter.If(isReturn, e => e.DeclareLocal(method.ReturnType));
                    ...
                    .If(!isInterface, c => c.CallBaseMethod(method)
                        .If(isReturn, e => e.SetReturnValue(method.ReturnType)))
                    .MarkLabel(lblCancel)
                ...
                .BeginCatchBlock(typeof(Exception))
                    ...
                    .If(AllowThrowException(attributes), e => e.ThrowException())
                .BeginFinallyBlock()
                    .CallInterceptors(context.InterceptMethodBuilder, InterceptType.Finally)
                .EndExceptionBlock()
                .If(isReturn, e1 => e1.GetReturnValue(method.ReturnType, lblRet), e1 => e1.ret());
                });
                ...
                ilCoding: ctx =>
                    {
                        ...
                        .ldloc(STACK_CALLINFO_INDEX)
                        .callvirt(MethodCache.CallInfoGetCancel).brtrue_s(lblCancel)
                        .If(isInterface, c => c.ldarg_0.ldfld(fieldBuilder.FieldBuilder), c => c.ldarg_0.call(method))
                ...
                ilCoding: ctx =>
                    {
                        ...
                        .ldloc(STACK_CALLINFO_INDEX)
                        .callvirt(MethodCache.CallInfoGetCancel).brtrue_s(lblCancel)
                        .If(isInterface, c => c.ldarg_0.ldarg_1.stfld(fieldBuilder.FieldBuilder), c => c.ldarg_0.GetArguments(property.PropertyType, 0).call(method))
                        .MarkLabel(lblCancel)
        ...
        private static EmitHelper InitLocal(this EmitHelper emitter, MemberInfo member, MethodInfo method = null)
        {
            return emitter
                ...
                .If(method != null, e => e.SetArguments(method));
        }
                ...
                .Each(
                    ...
                    (e, p, i) =>
                    {
                        ...
                        e.ldloc(STACK_ARGUMENT_INDEX)
                            ...
                            .ldarg(i + 1)
                            .If(isByref,
                                e1 => e1.If(type.IsValueType, e2 => e2.ldobj(type).box(type), e2 => e2.ldind_ref.end()),
                                e1 => e1.If(type.IsValueType || type.IsGenericParameter, e2 => e2.box(type)))
                            .stelem_ref
                            .end();
                    })
        ...
        private static EmitHelper SetCurrentMember(this EmitHelper emitter, MemberInfo member)
        {
            ...
            {
                //使用 ModuleType.GetProperty(propertyName) 获得属性
        ...
        private static EmitHelper SetBuilderResult(this EmitHelper emitter, AsyncStateMachineBuilder machineBuilder)
        {
            ...
            return emitter
                .If(machineBuilder.Result != null, e1 => e1.ldarg_0.call(machineBuilder.TrySetResultMethodBuilder.MethodBuilder))
                .ldarg_0
                .ldflda(machineBuilder.Builder)
                .If(machineBuilder.Result != null, e1 => e1.ldarg_0.ldfld(machineBuilder.Result))
                .call(mthSetResult);
        }
        ...
        private static AsyncStateMachineBuilder CreateAsyncStateMachine(BuildInternalContext context, MethodInfo method, DynamicMethodBuilder proxyBuilder)
        {
            ...
            typeBuilder.DefineMethod(nameof(IAsyncStateMachine.MoveNext), null, null, VisualDecoration.Public, ilCoding: b =>
            {
                ...
                b.Emitter
                    ...
                    .nop
                    .If(isValueTask, // todo 暂不知 ValueTask 为何不能与 Task 采取相同的IL
                        e1 => e1
                            .ldarg_0
                            .ldfld(machineBuilder.This)
                    ...
                    .If(machineBuilder.Result != null, e1 =>
                    ...
                    .BeginCatchBlock(typeof(Exception))
                        ...
                        .If(AllowThrowException(attributes), e => e.ldarg_0.ldfld(machineBuilder.CallInfo).call(MethodCache.CallInfoGetException).@throw.end())
                    .EndExceptionBlock()
                    .BeginCatchBlock(typeof(Exception))
